...
std::string joy_file_name = "/dev/input/js0";
...
private:
    ...
    void mainLoop_callback() {
        ...
        // for(int i=0; i<val.axes.size(); i++) {
        //     val.axes.at(i) = val.axes.at(i)*val.axes.at(i)*val.axes.at(i);
        // }
...
public:
    JoyNode() : Node("joy_node") {
        ...
        this->declare_parameter("file_name", joy_file_name);
        auto robot_port = this->get_parameter("file_name").as_string();

        joy = std::make_shared<Joystick>(robot_port.c_str());
