...
typedef struct recv_from_robot {
  ...
  union {
    struct {
      ...
      uint8_t stop_button : 1;
      uint8_t restart_button : 1;
      uint8_t proximity_back : 1;
      uint8_t proximity_avoid: 1;
      uint8_t proximity_silo : 1; //right LSB, left MSB
...
// joy_sub = this->create_subscription<sensor_msgs::msg::Joy>("joy",10,std::bind());
  ...
  intake_proximity_bottom_pub = this->create_publisher<std_msgs::msg::UInt8>("proximity_bottom",10);

  intake_proximity_middle_pub = this->create_publisher<std_msgs::msg::UInt8>("proximity_middle",10);

  intake_proximity_top_pub = this->create_publisher<std_msgs::msg::UInt8>("proximity_top",10);

  proximitysilo_pub = this->create_publisher<std_msgs::msg::UInt8>("proximity_silo",10);
  proximityback_pub = this->create_publisher<std_msgs::msg::UInt8>("proximity_back",10);
  proximityavoid_pub = this->create_publisher<std_msgs::msg::UInt8>("proximity_avoid",10);
  button_start_pub = this ->create_publisher<std_msgs::msg::UInt8>("button_start",10);
  button_stop_pub = this ->create_publisher<std_msgs::msg::UInt8>("button_stop",10);
  button_restart_pub = this ->create_publisher<std_msgs::msg::UInt8>("button_restart",10);

  tail_sub = this->create_subscription<std_msgs::msg::Float32>(
    "set_tail",10,std::bind(&robotNode::set_tail,this,std::placeholders::_1));

  pneumatic_sub = this->create_subscription<std_msgs::msg::Bool>(
    "set_pneumatic",10,std::bind(&robotNode::set_pneumatic,this,std::placeholders::_1));
...
void robotNode::set_pneumatic(const std_msgs::msg::Bool &msg) {
  send_data.ball_drib.pnuematic = (msg.data) ? 1 : 0;
}
...
int robotNode::udpLoop() {
  ...
  if (ret > 0) {  // jika tidak ada atau error, ret == -1
    ...
    std_msgs::msg::UInt8 intake_proximity_data_bottom;
    intake_proximity_data_bottom.data = recv_data.ball_drib.proximity_array[0];
    intake_proximity_bottom_pub->publish(intake_proximity_data_bottom);

    std_msgs::msg::UInt8 intake_proximity_data_middle;
    intake_proximity_data_middle.data = recv_data.ball_drib.proximity_array[1];
    intake_proximity_middle_pub->publish(intake_proximity_data_middle);

    std_msgs::msg::UInt8 intake_proximity_data_top;
    intake_proximity_data_top.data = recv_data.ball_drib.proximity_array[2];
    intake_proximity_top_pub->publish(intake_proximity_data_top);

    std_msgs::msg::UInt8 proximity_silo_data;
    proximity_silo_data.data = recv_data.proximity_silo;;
    proximitysilo_pub->publish(proximity_silo_data);

    std_msgs::msg::UInt8 proximity_back_data;
    proximity_back_data.data = recv_data.proximity_back;
    proximityback_pub->publish(proximity_back_data);

    std_msgs::msg::UInt8 proximity_avoid_data;
    proximity_avoid_data.data = (u_int8_t)(recv_data.proximity_avoid);
    proximityavoid_pub->publish(proximity_avoid_data);


    std_msgs::msg::UInt8 button_start_state;
    button_start_state.data = recv_data.start_button;
    button_start_pub->publish(button_start_state);

    std_msgs::msg::UInt8 button_stop_state;
    button_stop_state.data = recv_data.stop_button;
    button_stop_pub->publish(button_stop_state);

    std_msgs::msg::UInt8 button_restart_state;
    button_restart_state.data = recv_data.restart_button;
    button_restart_pub->publish(button_restart_state);
  }
  //return 0;
}
