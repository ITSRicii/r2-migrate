...
struct dataJoy{
    ...
    uint8_t l2;
    uint8_t r2;
    inline bool operator==(const dataJoy& rhs)
    {
        bool stat = true;
        stat &= button == rhs.button;
        stat &= lx == rhs.lx;
        stat &= ly == rhs.ly;
        stat &= rx == rhs.rx;
        stat &= ry == rhs.ry;
...
public:
    ...
    Joystick(char* file_fd) {
        ...
        data = dataJoy();
        file_str = std::string(file_fd);
        id = open(file_fd, O_RDONLY|O_NONBLOCK);
    };

    Joystick(const char* file_fd) {
        // joy.buttons.push_back(0);
        std::cout << "joy object creeeated!" << std::endl;
        data = dataJoy();
        file_str = std::string(file_fd);
    ...
    Joystick(char* led_id, char* bat_id) {
        ...
        file_str = std::string(default_file);
        led_class_id = std::string(led_id);
        battery_class_id = std::string(bat_id);
    }
    ...
    void reopen() {
        close(id);
        id = open(file_str.c_str(), O_RDONLY|O_NONBLOCK);
        std::cout << "joy reopen!" << std::endl;
    }
    ...
    int event() {
        ...
        // id = open(file_str.c_str(), O_RDONLY|O_NONBLOCK);
        // std::cout << id << " : " << file_str.c_str() << std::endl;
        // if(!id) {
        //     std::cout << "joy GONE!" << std::endl;
        //     close(id);
        //     return;
        // }
        ...
        if (r1 != -1) {
            ...
            for (int evi=0; evi<new_event_count; evi++) {
                ...
                switch (ev.type) {
                    case JS_EVENT_AXIS: {
                        switch (ev.number) {
                            case JS_AXIS_LX:   { data.lx = ev.value >> 8; if(data.lx == -128) data.lx = -127; repair(data.lx);} break;
                            case JS_AXIS_LY:   { data.ly = ev.value >> 8; if(data.ly == -128) data.ly = -127; repair(data.ly);} break;
                            case JS_AXIS_RX:   { data.rx = ev.value >> 8; if(data.rx == -128) data.rx = -127; repair(data.rx);} break;
                            case JS_AXIS_RY:   { data.ry = ev.value >> 8; if(data.ry == -128) data.ry = -127; repair(data.ry);} break;
                            case JS_AXIS_L2:   { data.l2 = (int8_t)(ev.value >> 8) + 128; uint16_t hys = data.button; repbit(data.button, JS_L2, (data.l2>(127+(cekbit(hys, JS_L2)?-27:27))));} break;
                            case JS_AXIS_R2:   { data.r2 = (int8_t)(ev.value >> 8) + 128; uint16_t hys = data.button; repbit(data.button, JS_R2, (data.r2>(127+(cekbit(hys, JS_R2)?-27:27))));} break;
